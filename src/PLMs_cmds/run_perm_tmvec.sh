...
OUTPUT_DIR="$TEMP_DIR/result_perm_src_tgt_tmvec_${query_basename}_to_${target_basename}"
mkdir -p "$OUTPUT_DIR"
...
set_target_db_path() {
    ...
    TMVEC_TARGET_DB_PATH="$BASE_DIR/data/db/db_${target_query_basename}_perm_src_tgt_tmvec"
    ...
    if [ ! -d "$TMVEC_TARGET_DB_PATH" ]; then
        ...
        CUDA_VISIBLE_DEVICES=$TMVEC_GPU_DEVICES python "${TMVEC_SRC_DIR}/scripts/tmvec-build-database-perm" \
...
run_tmvec_search() {
    ...
    CUDA_VISIBLE_DEVICES=$TMVEC_GPU_DEVICES python "${TMVEC_SRC_DIR}/scripts/tmvec-search-perm" \
