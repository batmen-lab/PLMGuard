...
OUTPUT_DIR="$TEMP_DIR/result_perm_2layer_dctdomain_${query_basename}_to_${target_basename}"
...
set_dctdomain_target_db_path() {
    ...
    DCT_DOMAIN_TARGET_DB_PATH="$DATA_DIR/db/db_${target_basename}_perm_2layer_dctdomain/${target_basename}_db.db"
    ...
    if [ ! -f "$DCT_DOMAIN_TARGET_DB_PATH" ]; then
        ...
        CUDA_VISIBLE_DEVICES=$DCT_DOMAIN_GPU_DEVICES python ${DCT_DOMAIN_SRC_DIR}/src/make_db_perm.py \
...
run_dctdomain_search() {
    ...
    python ${DCT_DOMAIN_SRC_DIR}/src/query_db_perm.py \
