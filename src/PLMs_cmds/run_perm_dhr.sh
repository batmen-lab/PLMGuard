...
OUTPUT_DIR="$TEMP_DIR/result_perm_dhr_${query_basename}_to_${target_basename}"
...
set_target_db_path() {
    ...
    if [ -z "$TARGET_TSV_PATH" ]; then
        ...
        DHR_TARGET_DB_DIR_PATH="$DATA_DIR/db/db_${target_basename}_perm_dhr"
        
        if [ ! -d "$DHR_TARGET_DB_DIR_PATH/agg" ]; then
            echo "Creating directory for ${target_basename} database..."
            ls $TARGET_TSV_PATH
            
            CUDA_VISIBLE_DEVICES=$DHR_GPU_DEVICES python ${DHR_SRC_DIR}/do_embedding_perm.py trainer.ur90_path=$(realpath "$TARGET_TSV_PATH") model.ckpt_path=$CKPT_PATH hydra.run.dir=$DHR_TARGET_DB_DIR_PATH \
...
CUDA_VISIBLE_DEVICES=$DHR_GPU_DEVICES \
    python ${DHR_SRC_DIR}/do_retrieval_perm.py \
    -i "$QUERY_TSV_PATH" \
