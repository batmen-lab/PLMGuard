...
OUTPUT_DIR="$TEMP_DIR/result_perm_plm_${query_basename}_to_${target_basename}"
mkdir -p "$OUTPUT_DIR"
...
set_target_db_path() {
    ...
    if [ -z "$TARGET_FA_PATH" ]; then
        ...
        PLMSEARCH_TARGET_DB_PATH="$BASE_DIR/data/db/db_${target_basename}_perm_plm/db_${target_basename}_embedding.pkl"
        ...
        if [ ! -f "$PLMSEARCH_TARGET_DB_PATH" ]; then
           ...
           CUDA_VISIBLE_DEVICES=$PLMSEARCH_GPU_DEVICES python "$PLMSEARCH_SRC_DIR/plmsearch/embedding_generate_perm.py" \
...
run_plmsearch() {
    ...
    query_embed="$BASE_DIR/data/db/db_astral_perm_plm/${embed_name}_embedding.pkl"
